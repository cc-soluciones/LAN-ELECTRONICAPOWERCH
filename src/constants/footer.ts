import { name } from "@/constants/general";

export const description = `Especialistas en instalaciones fotovoltaicas solares de alta eficiencia para hogares e industrias en Chile.`;
export const coverage = `Cobertura: Todo Chile `;
export const warranty = `Garantía: 2 años`;
export const support = `Soporte: 24/7`;
export const copy = `© ${new Date().getFullYear()} ${name}. Todos los derechos reservados.`